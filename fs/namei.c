...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (is_nd_state_lookup_last_and_open_last && dentry && !IS_ERR(dentry) && d_backing_inode(dentry) &&
			susfs_is_inode_sus_path(d_backing_inode(dentry)))
		{
			if (d_in_lookup(dentry))
				d_lookup_done(dentry);
			dentry = NULL;
			...
			bool got_write, int *opened)
{
	...
	*opened &= ~FILE_CREATED;
	dentry = d_lookup(dir, &nd->last);
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if ((nd->state & ND_STATE_OPEN_LAST) && dentry && !IS_ERR(dentry) && d_backing_inode(dentry) &&
		susfs_is_inode_sus_path(d_backing_inode(dentry)))
	{
		if (d_in_lookup(dentry))
			d_lookup_done(dentry);
