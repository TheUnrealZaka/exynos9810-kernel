...
typedef union {
	...
	uint8_t  BYTES[4];
	struct {
	    uint32_t	Vdm_Flag_Reserve_b0:1,		
			Vdm_Flag_Discover_ID:1,		
			Vdm_Flag_Discover_SVIDs:1,	
			Vdm_Flag_Discover_MODEs:1,	
			Vdm_Flag_Enter_Mode:1,		
			Vdm_Flag_Exit_Mode:1,		
			Vdm_Flag_Attention:1,		
			Vdm_Flag_Reserved:9,		
			Vdm_Flag_DP_Status_Update:1,	
			Vdm_Flag_DP_Configure:1,	
			Vdm_Flag_Reserved2:14;		
	} BITS;
} MAX77705_VDM_MSG_IRQ_STATUS_Type;

struct DP_DP_DISCOVER_IDENTITY { 
	SEND_VDM_BYTE_DATA byte_data;
	UND_DATA_MSG_VDM_HEADER_Type vdm_header;

} __attribute__((aligned(1), packed));

struct DP_DP_DISCOVER_ENTER_MODE { 
	SEND_VDM_BYTE_DATA byte_data;
	UND_DATA_MSG_VDM_HEADER_Type vdm_header;
...
struct DP_DP_CONFIGURE { 
	SEND_VDM_BYTE_DATA byte_data;
	UND_DATA_MSG_VDM_HEADER_Type vdm_header;
	UND_DP_CONFIG_UPDATE_Type vdo_config;
} __attribute__((aligned(1), packed));

struct SS_DEX_DISCOVER_MODE { 
	SEND_VDM_BYTE_DATA byte_data;
	UND_DATA_MSG_VDM_HEADER_Type vdm_header;
} __attribute__((aligned(1), packed));

struct SS_DEX_ENTER_MODE { 
	SEND_VDM_BYTE_DATA byte_data;
	UND_DATA_MSG_VDM_HEADER_Type vdm_header;
} __attribute__((aligned(1), packed));
